...
function Spectrogram({ confidence }) {
  ...
  const draw = useCallback(() => {
    ...
    for (let x = 0; x < W; x++) {
      ...
      // ── The threat tonal at ~42 Hz (normalized: 0.14 of range) ──
      ...
      if (dist < spread * 4) {
        ...
        intensity += conf * 0.85 * g * (0.75 + 0.25 * Math.sin(t * 0.09))
      }
      // Harmonics
      for (let h = 2; h <= 4; h++) {
...
function BearingRose({ bearing }) {
  ...
  return (
    <svg width={88} height={88} viewBox="0 0 88 88">
      ...
      <line x1={cx} y1={cy} x2={x2} y2={y2} stroke="#ef4444" strokeWidth="2.5" strokeLinecap="round" />
...
// ── Main component ────────────────────────────────────────────────
export default function HydrophonePanel({ confidence, threatRange }) {
    ...
    <div className="panel flex flex-col h-full">
      ...
      <div className="panel-header justify-between">
        ...
        <div className="flex items-center gap-3">
          <span className="font-mono text-[10px] text-[#334155]">48-element array · 87 m depth</span>
      ...
      <div className="flex-1 flex flex-col gap-0 overflow-hidden">
        ...
        <div className="px-5 pt-3 pb-1">
          ...
          <Spectrogram confidence={confidence} />
        </div>
        ...
        <div className="flex items-start gap-6 px-5 py-3 border-t border-[#1a3050] mt-auto">
          ...
          <div className="flex-shrink-0 w-28">
            <div className="font-mono text-[9px] text-[#334155] mb-2">CONFIDENCE</div>
            <div className="font-mono text-[32px] font-bold text-[#ef4444] leading-none">{pct}%</div>
            <div className="mt-2 threat-track">
              <div
                className="threat-fill"
                style={{
                  width: `${pct}%`,
                  background: pct > 70 ? '#ef4444' : pct > 50 ? '#eab308' : '#22c55e',
                }}
              />
            </div>
            <div className="font-mono text-[9px] mt-1.5"
              style={{ color: pct > 70 ? '#ef4444' : '#eab308' }}>
              {pct > 70 ? 'HIGH CONFIDENCE' : 'MODERATE'}
            </div>
          </div>
        </div>
      </div>
